...
class _FilterPageBodyState extends State<FilterPageBody> {
  ...
  void initState() {
    ...
    if (homeCubit.filterSkills.isEmpty) {
      homeCubit.addSkill();
    }
  }
        ...
        return ListView(
          ...
          children: [
            ...
            FilterDropdownSection(
              title: "طبيعة العمل",
              hint: "طبيعة العمل",
              items: jobTypeMap.keys.toList(),
              value: jobTypeMap.entries
                  .firstWhere((e) => e.value == homeCubit.filterJobType, orElse: () => const MapEntry('', ''))
                  .key,
              onChanged: (value) {
                if (value != null) {
                  homeCubit.filterJobType = jobTypeMap[value]!;
                }
              },
            ),
            FilterDropdownSection(
              ...
              hint: "نوع مكان العمل",
              items: workPlaceTypeMap.keys.toList(),
              value: workPlaceTypeMap.entries
                  .firstWhere((e) => e.value == homeCubit.filterWorkPlaceType, orElse: () => const MapEntry('', ''))
                  .key,
              onChanged: (value) {
                if (value != null) {
                  homeCubit.filterWorkPlaceType = workPlaceTypeMap[value]!;
                }
              },
            ),
            FilterDropdownSection(
              ...
              value: experienceLevelMap.entries
                  .firstWhere((e) => e.value == homeCubit.filterExperienceLevel, orElse: () => const MapEntry('', ''))
                  .key,
              onChanged: (value) {
                if (value != null) {
